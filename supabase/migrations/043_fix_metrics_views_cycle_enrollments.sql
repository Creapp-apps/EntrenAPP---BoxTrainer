-- ============================================================
-- MIGRATION 043 — Fix Metrics RPCs after Cycle Enrollments Refactor
-- Updates trainer_analytics_stats and student_adherence_list to query
-- training_cycle_enrollments instead of training_cycles.student_id
-- ============================================================

-- 1. Update trainer_analytics_stats
...
BEGIN
  ...
  SELECT
    count(DISTINCT c.id) FILTER (WHERE COALESCE(c.cycle_type, 'strength') = 'strength'),
    count(DISTINCT c.id) FILTER (WHERE c.cycle_type = 'crossfit')
  ...
  WITH planned AS (
    SELECT DISTINCT d.id AS day_id, tce.student_id
    FROM public.training_cycles c
    JOIN public.training_cycle_enrollments tce ON tce.cycle_id = c.id
    JOIN public.users u ON u.id = tce.student_id
...
-- 2. Update student_adherence_list
...
BEGIN
  ...
  RETURN QUERY
  WITH planned AS (
    SELECT tce.student_id, COALESCE(c.cycle_type, 'strength') AS ctype, d.id AS day_id
    FROM public.training_cycles c
    JOIN public.training_cycle_enrollments tce ON tce.cycle_id = c.id
    JOIN public.users u ON u.id = tce.student_id
    JOIN public.training_weeks w ON w.cycle_id = c.id
