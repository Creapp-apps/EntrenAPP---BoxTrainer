-- MIGRACIÓN 049 — Soporte de Variantes para Ejercicios de CrossFit (cf_exercises)

-- 1. Crear tabla cf_exercise_variants
CREATE TABLE IF NOT EXISTS public.cf_exercise_variants (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  exercise_id uuid REFERENCES public.cf_exercises(id) ON DELETE CASCADE NOT NULL,
  name text NOT NULL,
...
-- 2. Habilitar RLS
ALTER TABLE public.cf_exercise_variants ENABLE ROW LEVEL SECURITY;

-- 3. Crear Políticas de RLS para cf_exercise_variants
DROP POLICY IF EXISTS "read_cf_exercise_variants" ON public.cf_exercise_variants;
CREATE POLICY "read_cf_exercise_variants" ON public.cf_exercise_variants FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM public.cf_exercises e
      WHERE e.id = exercise_id
    )
  );

DROP POLICY IF EXISTS "manage_cf_exercise_variants" ON public.cf_exercise_variants;
CREATE POLICY "manage_cf_exercise_variants" ON public.cf_exercise_variants FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM public.cf_exercises e
      WHERE e.id = exercise_id
...
-- 4. Agregar variant_id a cf_block_exercises
ALTER TABLE public.cf_block_exercises
  ADD COLUMN IF NOT EXISTS variant_id uuid REFERENCES public.cf_exercise_variants(id) ON DELETE SET NULL;

-- 5. Recrear copy_cycle para incluir variant_id en cf_block_exercises
    ...
    LOOP
      ...
      LOOP
        ...
        FOR v_ex IN
          ...
          ORDER BY "order"
        -- Con el fix de complexes de fuerza
        LOOP
        ...
        LOOP
          ...
          INSERT INTO public.cf_block_exercises (
            id, block_id, exercise_id, variant_id, "order", reps, unit_override, notes
          ) VALUES (
            v_new_cf_ex_id,
            v_new_block_id,
            v_cf_ex.exercise_id,
