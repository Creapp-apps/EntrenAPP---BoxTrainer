    ...
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-border z-50" style={{ paddingBottom: "env(safe-area-inset-bottom)" }}>
      <div className="flex items-center justify-start gap-2 px-4 py-2 overflow-x-auto flex-nowrap [&::-webkit-scrollbar]:hidden">
        {navItems.map(({ href, label, icon: Icon, exact }) => {
          ...
          return (
            <Link key={href} href={href}
              className={cn(
                "flex flex-col items-center gap-1 px-3 py-2 rounded-xl transition-colors shrink-0",
        ...
        <button onClick={handleLogout}
          className="flex flex-col items-center gap-1 px-3 py-2 rounded-xl text-muted-foreground hover:text-destructive transition-colors shrink-0 ml-auto">
