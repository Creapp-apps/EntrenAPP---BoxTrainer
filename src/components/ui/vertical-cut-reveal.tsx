...
const VerticalCutReveal = forwardRef<VerticalCutRevealRef, TextProps>(
  (
    ...
    const elements = useMemo(() => {
      ...
      return splitBy === "words" ? text.split(" ") : splitBy === "lines" ? text.split(" ") : text.split(splitBy)
    }, [text, splitBy])
    ...
    const getStaggerDelay = useCallback(
      (index: number) => {
        const total =
          splitBy === "characters"
            ? elements.reduce(
                (acc, word) => acc + (typeof word === "string" ? 1 : (word as WordObject).characters.length + ((word as WordObject).needsSpace ? 1 : 0)),
                0
              )
            : elements.length
    ...
    }, [autoStart, animate, startAnimation])
    ...
    return (
      ...
      >
        ...
        {(splitBy === "characters"
          ...
          return (
            <span key={wordIndex} aria-hidden="true" className={cn("inline-flex overflow-hidden pb-[0.2em] -mb-[0.2em]", wordLevelClassName)}>
